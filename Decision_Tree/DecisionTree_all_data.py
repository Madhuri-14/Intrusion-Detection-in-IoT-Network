''' This Program is to read multiple Mirai.csv from from the folder along with the benign file
...
def dataset_training():
   ...
   # attack_file = sys.argv[2]
    ...
    # Reading File
    df_benign = pd.read_csv("C:\\Master's\\Spring 2020\\Graduate Research Assistanship\\Dataset\\iot-network-intrusion-dataset-CSV-FORMAT\\benign\\benign-dec.csv")
    filenames = glob("C:\\Master's\\Spring 2020\\Graduate Research Assistanship\\Dataset\\iot-network-intrusion-dataset-CSV-FORMAT\\*.csv") 
    #('/Users/Phani/Desktop/sales*.csv')
    attack_list = list()
    for f in filenames:
        attack_list.append(pd.read_csv(f))
        #df_attack = pd.concat([df_attack, pd.read_csv(f)])
...
def DTDepth_training():
    ...
    df_attack = [pd.read_csv(f) for f in filenames]
